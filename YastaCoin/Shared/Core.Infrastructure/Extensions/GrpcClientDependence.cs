...
namespace Core.Infrastructure.Extensions
{
    public static class GrpcClientDependence
    {
        public static IHttpClientBuilder AddGrpcClientDependence<T>(this IServiceCollection services, string name, IConfiguration configuration, string port = "") where T : class
        {
            ...
            // Circuit breaker opcional: se engancha sólo si el servicio destino
            // declara la sección CircuitBreaker en su appsettings. Mismo criterio
            // que PolicyConfiguration: el mecanismo acá, la política allá.
            if (configuration.GetSection("CircuitBreaker").Exists())
            {
                client.AddCircuitBreakerDependence(name, configuration.GetSection("CircuitBreaker"));
            }
            return client;
        }
    }
