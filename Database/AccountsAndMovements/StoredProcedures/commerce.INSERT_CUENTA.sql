...
--              se asienta un CREDITO de apertura en commerce.MOVIMIENTO dentro
--              de la misma transaccion. Asi el libro mayor explica el saldo
--              desde el primer segundo y la cuenta nunca tiene plata sin
--              respaldo.
--
-- Parametros:
--   @CUEN_TITULAR_TIPO_VC   VARCHAR(20)    'CLIENT' o 'COMMERCE'
...
IF EXISTS (
    ...
    WHERE s.name = 'commerce' AND p.name = 'INSERT_CUENTA'
)
    DROP PROCEDURE commerce.INSERT_CUENTA;
GO
...
CREATE PROCEDURE commerce.INSERT_CUENTA
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SET @Json = (SELECT * FROM commerce.MOVIMIENTO WHERE MOVI_ID_IT = @AperturaId
                         FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);

            EXEC aud.REGISTRAR_AUDITORIA 'commerce', 'MOVIMIENTO', @AperturaId, 'INSERT',
        ...
        SET @Json = (SELECT * FROM commerce.CUENTA WHERE CUEN_ID_IT = @CuentaId
                     FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);

        EXEC aud.REGISTRAR_AUDITORIA 'commerce', 'CUENTA', @CuentaId, 'INSERT',
             NULL, @Json, @AUDITORIA_USUARIO_IT, @AUDITORIA_TRAZA_VC;
    ...
    BEGIN CATCH
        ...
        -- UQ_COMMERCE_CUENTA_TITULAR: el titular ya tiene cuenta en esa moneda.
        -- Es una regla de negocio, no una falla del sistema.
