...
--   ataja UQ_COMMERCE_MOVIMIENTO_IDEMPOTENCIA y se resuelve en el CATCH.
--
-- QR DE UN SOLO USO
--   Lo garantiza UQ_COMMERCE_MOVIMIENTO_QR, no una llamada al servicio de QR:
--   si dos clientes distintos pagan el mismo QR a la vez, el motor deja pasar
--   un unico debito y el otro recibe -6.
--
-- Parametros:
...
IF EXISTS (
    ...
    WHERE s.name = 'commerce' AND p.name = 'EJECUTAR_PAGO_QR'
)
    DROP PROCEDURE commerce.EJECUTAR_PAGO_QR;
GO

-- Los INSERT sobre commerce.MOVIMIENTO tocan el indice filtrado
-- UQ_COMMERCE_MOVIMIENTO_QR, que exige QUOTED_IDENTIFIER ON. El valor queda
-- grabado JUNTO al procedimiento al crearlo: SSMS lo trae activado, pero sqlcmd
-- lo deja OFF, y sin esta linea el SP se crea igual y recien falla al
-- ejecutarse (Msg 1934).
SET QUOTED_IDENTIFIER ON;
SET ANSI_NULLS ON;
GO

CREATE PROCEDURE commerce.EJECUTAR_PAGO_QR
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM   commerce.CUENTA WITH (UPDLOCK, HOLDLOCK)
        ...
        IF EXISTS (
            SELECT 1 FROM commerce.MOVIMIENTO
        ...
        INSERT INTO commerce.MOVIMIENTO (
        ...
        -- restriccion UQ_COMMERCE_MOVIMIENTO_IDEMPOTENCIA es GLOBAL sobre la
        -- clave sola, asi que sin el sufijo el credito chocaria contra el debito
        -- en todos los pagos, no solo en un caso raro.
        INSERT INTO commerce.MOVIMIENTO (
            MOVI_CUENTA_ID_IT, MOVI_TIPO_VC, MOVI_ESTADO_VC,
            MOVI_MONTO_DE, MOVI_SALDO_ANTERIOR_DE, MOVI_SALDO_POSTERIOR_DE,
            MOVI_MONTO_ORIGEN_DE, MOVI_MONEDA_ORIGEN_VC, MOVI_TIPO_CAMBIO_DE,
        ...
        SET @OrigenAntes  = (SELECT * FROM commerce.CUENTA WHERE CUEN_ID_IT = @OrigenId
                             FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);
        SET @DestinoAntes = (SELECT * FROM commerce.CUENTA WHERE CUEN_ID_IT = @DestinoId
                             FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);

        UPDATE commerce.CUENTA
        ...
        SET @OrigenDespues  = (SELECT * FROM commerce.CUENTA WHERE CUEN_ID_IT = @OrigenId
                               FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);
        SET @DestinoDespues = (SELECT * FROM commerce.CUENTA WHERE CUEN_ID_IT = @DestinoId
                               FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);

        -- Los JSON pasan por variables: T-SQL no admite una subconsulta como
        -- argumento de EXEC.
        SET @DebitoJson  = (SELECT * FROM commerce.MOVIMIENTO WHERE MOVI_ID_IT = @Resultado
                            FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);
        SET @CreditoJson = (SELECT * FROM commerce.MOVIMIENTO WHERE MOVI_ID_IT = @CreditoId
                            FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);

        EXEC aud.REGISTRAR_AUDITORIA 'commerce', 'MOVIMIENTO', @Resultado, 'INSERT',
             NULL, @DebitoJson, @AUDITORIA_USUARIO_IT, @AUDITORIA_TRAZA_VC;

        EXEC aud.REGISTRAR_AUDITORIA 'commerce', 'MOVIMIENTO', @CreditoId, 'INSERT',
             NULL, @CreditoJson, @AUDITORIA_USUARIO_IT, @AUDITORIA_TRAZA_VC;

        EXEC aud.REGISTRAR_AUDITORIA 'commerce', 'CUENTA', @OrigenId, 'UPDATE',
             @OrigenAntes, @OrigenDespues,
             @AUDITORIA_USUARIO_IT, @AUDITORIA_TRAZA_VC;

        EXEC aud.REGISTRAR_AUDITORIA 'commerce', 'CUENTA', @DestinoId, 'UPDATE',
    ...
    BEGIN CATCH
        ...
        BEGIN
            ...
            -- Carrera contra UQ_COMMERCE_MOVIMIENTO_QR: otro cliente pago el
            -- mismo QR primero. No es un error del sistema, es el QR ya usado.
            SELECT CAST(-6 AS BIGINT) AS Resultado;
            RETURN;
        END;
