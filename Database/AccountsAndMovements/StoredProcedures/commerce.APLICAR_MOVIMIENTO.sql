-- =============================================================================
-- SP:          commerce.APLICAR_MOVIMIENTO
...
--              commerce.MOVIMIENTO y actualiza commerce.CUENTA en la misma
--              transaccion. Es la unica via por la que cambia el saldo
--              fuera del pago QR (recargas, ajustes, reversas).
--
...
IF EXISTS (
    ...
    WHERE s.name = 'commerce' AND p.name = 'APLICAR_MOVIMIENTO'
)
    DROP PROCEDURE commerce.APLICAR_MOVIMIENTO;
GO
...
CREATE PROCEDURE commerce.APLICAR_MOVIMIENTO
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- id de la recarga y el debito no se aplicaria nunca, sin que el llamador
        -- se entere. Es el mismo criterio que usa commerce.EJECUTAR_PAGO_QR.
        ...
        SET @MovimientoJson = (SELECT * FROM commerce.MOVIMIENTO WHERE MOVI_ID_IT = @Resultado
                               FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);

        EXEC aud.REGISTRAR_AUDITORIA
             'commerce', 'MOVIMIENTO', @Resultado, 'INSERT',
             NULL, @MovimientoJson,
             @AUDITORIA_USUARIO_IT, @AUDITORIA_TRAZA_VC;

        -- La cuenta antes y despues del cambio de saldo.
        SET @CuentaAntes = (SELECT * FROM commerce.CUENTA WHERE CUEN_ID_IT = @CuentaId
                            FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);
        ...
        SET @CuentaDespues = (SELECT * FROM commerce.CUENTA WHERE CUEN_ID_IT = @CuentaId
                              FOR JSON PATH, WITHOUT_ARRAY_WRAPPER);

        EXEC aud.REGISTRAR_AUDITORIA
             'commerce', 'CUENTA', @CuentaId, 'UPDATE',
    ...
    BEGIN CATCH
        ...
        -- Carrera contra UQ_COMMERCE_MOVIMIENTO_IDEMPOTENCIA: otro hilo aplico la
