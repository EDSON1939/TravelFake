...
-- Schema: commerce
-- Tabla:  CUENTA
-- Descripcion: Cuentas con saldo. El titular puede ser un CLIENT extranjero
--              (opera en su moneda: PEN, USD, EUR...) o un COMMERCE boliviano
...
--   _DT  = DATETIME
--
-- Nota: CUEN_TITULAR_ID_IT no lleva FK. CLIENT y COMMERCE son propiedad de
...
IF NOT EXISTS (
    ...
    WHERE s.name = 'commerce' AND t.name = 'CUENTA'
)
BEGIN
    CREATE TABLE commerce.CUENTA (
        ...
        CONSTRAINT PK_COMMERCE_CUENTA         PRIMARY KEY (CUEN_ID_IT),
        CONSTRAINT UQ_COMMERCE_CUENTA_NUMERO  UNIQUE      (CUEN_NUMERO_VC),
        -- Un titular tiene a lo sumo una cuenta por moneda.
        CONSTRAINT UQ_COMMERCE_CUENTA_TITULAR UNIQUE
        ...
        CONSTRAINT CK_COMMERCE_CUENTA_SALDO   CHECK (CUEN_SALDO_DE >= 0),
        CONSTRAINT CK_COMMERCE_CUENTA_TITULAR CHECK (CUEN_TITULAR_TIPO_VC IN ('CLIENT', 'COMMERCE'))
    );

    -- Resolucion (titular, moneda) -> cuenta, que es la lectura del pago.
    CREATE NONCLUSTERED INDEX IX_COMMERCE_CUENTA_TITULAR
        ON commerce.CUENTA (CUEN_TITULAR_TIPO_VC, CUEN_TITULAR_ID_IT)
        INCLUDE (CUEN_NUMERO_VC, CUEN_MONEDA_ID_IT, CUEN_MONEDA_CODIGO_VC,
                 CUEN_SALDO_DE, CUEN_ACTIVO_BT);

    PRINT 'Tabla commerce.CUENTA creada correctamente.';
END
ELSE
BEGIN
    PRINT 'Tabla commerce.CUENTA ya existe.';
END
GO
