...
--              pay.CUENTA siempre puede reconstruirse desde aqui.
--
...
IF NOT EXISTS (
    ...
    WHERE s.name = 'pay' AND t.name = 'MOVIMIENTO'
)
BEGIN
    CREATE TABLE pay.MOVIMIENTO (
        ...
        CONSTRAINT PK_PAY_MOVIMIENTO        PRIMARY KEY (MOVI_ID_IT),
        CONSTRAINT CK_PAY_MOVIMIENTO_MONTO  CHECK       (MOVI_MONTO_DE > 0),
        CONSTRAINT CK_PAY_MOVIMIENTO_TIPO   CHECK       (MOVI_TIPO_VC IN ('CREDITO', 'DEBITO')),
        CONSTRAINT CK_PAY_MOVIMIENTO_ESTADO CHECK       (MOVI_ESTADO_VC IN ('PENDING', 'COMPLETED', 'FAILED', 'CANCELLED')),
        CONSTRAINT CK_PAY_MOVIMIENTO_CAMBIO CHECK       (MOVI_TIPO_CAMBIO_DE > 0),
        CONSTRAINT FK_PAY_MOVIMIENTO_CUENTA FOREIGN KEY (MOVI_CUENTA_ID_IT)
            REFERENCES pay.CUENTA (CUEN_ID_IT),
        ...
        CONSTRAINT UQ_PAY_MOVIMIENTO_IDEMPOTENCIA UNIQUE (MOVI_IDEMPOTENCIA_VC)
    );

    -- Extracto de la cuenta, mas reciente primero.
    CREATE NONCLUSTERED INDEX IX_PAY_MOVIMIENTO_CUENTA_FECHA
        ON pay.MOVIMIENTO (MOVI_CUENTA_ID_IT, MOVI_FECHA_CREACION_DT DESC)
    ...
    CREATE NONCLUSTERED INDEX IX_PAY_MOVIMIENTO_TRANSACCION
        ON pay.MOVIMIENTO (MOVI_TRANSACCION_VC);

    PRINT 'Tabla pay.MOVIMIENTO creada correctamente.';
END
ELSE
BEGIN
    PRINT 'Tabla pay.MOVIMIENTO ya existe.';
END
GO
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UQ_PAY_MOVIMIENTO_QR')
    CREATE UNIQUE NONCLUSTERED INDEX UQ_PAY_MOVIMIENTO_QR
        ON pay.MOVIMIENTO (MOVI_QR_CODIGO_VC)
...
-- UQ_PAY_MOVIMIENTO_IDEMPOTENCIA ya es un indice unico sobre esa columna y
