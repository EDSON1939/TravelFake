...
public class UserRepository(IQuery query, ICommand command, IAuditContext audit) : IUserRepository
{
    ...
    public async Task<long> Insert(UserEntity entity, CancellationToken ct = default)
        => await command.ExecuteAsync(new InsertUserCommand(entity, audit), ct);
    ...
    public async Task<long> RegisterLoginAttempt(
        ...
        => await command.ExecuteAsync(
            new RegisterLoginAttemptCommand(userId, succeeded, maxAttempts, lockMinutes, audit), ct);

    public async Task<long> Delete(long userId, CancellationToken ct = default)
        => await command.ExecuteAsync(new DeleteUserCommand(userId, audit), ct);
}
