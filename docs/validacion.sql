...
INSERT INTO #r EXEC pay.INSERT_CUENTA 'CLIENTE', 1, 2, 'USD', 500, 'traza-alta-1', 7;
SELECT @c1 = v FROM #r; DELETE #r;
INSERT INTO #r EXEC pay.INSERT_CUENTA 'COMERCIO', 55, 1, 'BOB', 0, 'traza-alta-2', 7;
...
PRINT '=== 2. Pago QR ===';
DECLARE @pago BIGINT;
INSERT INTO #r EXEC pay.EJECUTAR_PAGO_QR
...
SELECT CASE WHEN (SELECT CUEN_SALDO_DE FROM pay.CUENTA WHERE CUEN_ID_IT=1) = 480.00
             AND (SELECT CUEN_SALDO_DE FROM pay.CUENTA WHERE CUEN_ID_IT=2) = 139.20
...
DECLARE @rec BIGINT;
INSERT INTO #r EXEC pay.APLICAR_MOVIMIENTO 'QR0000000001','CREDITO',100,'RECARGA','TOPUP-001','Recarga','traza-rec',7;
SELECT @rec = v FROM #r; DELETE #r;
SELECT CASE WHEN @rec > 0 AND (SELECT CUEN_SALDO_DE FROM pay.CUENTA WHERE CUEN_ID_IT=1) = 580.00
            THEN 'OK  recarga aplicada, saldo 580' ELSE 'FALLO recarga' END AS T11;

INSERT INTO #r EXEC pay.APLICAR_MOVIMIENTO 'QR0000000001','CREDITO',100,'RECARGA','TOPUP-001','Recarga','traza-rec2',7;
SELECT CASE WHEN (SELECT v FROM #r) = @rec AND (SELECT CUEN_SALDO_DE FROM pay.CUENTA WHERE CUEN_ID_IT=1) = 580.00
...
INSERT INTO #r EXEC pay.APLICAR_MOVIMIENTO 'QR0000000001','DEBITO',500,'AJUSTE','TX-2026-000001','Ajuste','traza-mal2',7;
...
INSERT INTO #r EXEC pay.APLICAR_MOVIMIENTO 'QR0000000001','CREDITO',999,'X','TOPUP-001','Otro monto','traza-mal3',7;
SELECT CASE WHEN (SELECT v FROM #r) = -9 THEN 'OK  misma clave con otro monto -> -9'
            ELSE 'FALLO monto distinto: ' + CAST((SELECT v FROM #r) AS VARCHAR) END AS T14;
DELETE #r;

PRINT '=== 5. Saldo insuficiente y rollback de auditoria ===';
INSERT INTO #r EXEC pay.APLICAR_MOVIMIENTO 'QR0000000001','DEBITO',99999,'X','TX-SINSALDO','Sin saldo','traza-sinsaldo',7;
...
FROM (
  SELECT c.CUEN_ID_IT
  FROM pay.CUENTA c
  LEFT JOIN pay.MOVIMIENTO m ON m.MOVI_CUENTA_ID_IT = c.CUEN_ID_IT
